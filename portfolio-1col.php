...
get_header(); ?>
	...
	<div id="content" class="portfolio portfolio-one portfolio-one-text <?php echo $content_class; ?>" style="<?php echo $content_css; ?>">
		...
		<ul class="portfolio-tabs clearfix">
			...
			<?php if(get_post_meta(get_the_ID(), 'wired_portfolio_category', true)  && !in_array('0', get_post_meta(get_the_ID(), 'wired_portfolio_category', true))): ?>
			<?php if(in_array($portfolio_cat->term_id, get_post_meta(get_the_ID(), 'wired_portfolio_category', true))): ?>
		...
		<div class="portfolio-wrapper">
			...
			$pcats = get_post_meta(get_the_ID(), 'wired_portfolio_category', true);
			if($pcats && $pcats[0] == 0) {
				unset($pcats[0]);
			}
			if($pcats){
			...
			<div class="portfolio-item <?php echo $item_classes; ?>">
				...
				<?php elseif(!has_post_thumbnail() && get_post_meta($post->ID, 'wired_video', true)): ?>
				...
				<div class="portfolio-content">
					...
					<?php
					if(get_post_meta($current_page_id, 'wired_portfolio_excerpt', true)) {
						$excerpt_length = get_post_meta($current_page_id, 'wired_portfolio_excerpt', true);
					} else {
					...
					<div class="buttons">
						<a href="<?php echo $permalink; ?>" class="green button small"><?php echo __('Learn More', 'Inhouse'); ?></a>
